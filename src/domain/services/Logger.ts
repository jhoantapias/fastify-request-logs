...
export class Logger implements Printer, Finisher {
  ...
  public finish(
    ...
    try {
      if (!isError) {
        this.data.RESPONSE =
          typeof payload === 'string' && payload ? JSON.parse(payload) : payload;
      ...
      const color = statusCode >= 200 && statusCode <= 300 ? COLORS.CYAN : COLORS.RED;
  ...
  constructor(req: FastifyRequestModel) {
